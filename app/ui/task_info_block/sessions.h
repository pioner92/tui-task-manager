    ...
    for (const auto& s: task_sessions) {
        ...
        rows.push_back(
            hbox({
                ...
                text(" "),
                s.end_at.has_value() ? hbox({text("| "), text(format_sec_to_hhmmss(get_session_duration_sec(s)))})
                                     : hbox(),
                PaddingH(),
            }) |
            (!s.end_at.has_value() ? color(theme::cyan) | bold : color(theme::muted)));
    }
    return vbox({hbox({
